...
def baixar_documentos(driver, links, files_name, pasta):
    original_window = driver.current_window_handle 
    for index, (link, name) in enumerate(zip(links, files_name)):
        driver.switch_to.new_window('tab')
        driver.get(link)
        with open(os.path.join(pasta, f"documento_{name}.html"), 'w', encoding='utf-8', errors='ignore') as file:
            file.write(driver.page_source)        
        driver.close()
        driver.switch_to.window(original_window)

def processar_htmls(pasta, dir_saida="."):
